...
zmq::sspi_client_t::sspi_client_t (session_base_t *session_,
                                       const options_t &options_) :
    mechanism_base_t (session_, options_),
    sspi_mechanism_base_t (session_, options_),
    service_name (options_.gss_service_principal),
...
zmq::sspi_client_t::~sspi_client_t ()
{
    ...
    // if (send_tok.pvBuffer)
    //      FreeContextBuffer ...
}

int zmq::sspi_client_t::next_handshake_command (msg_t *msg_)
{
    if (state == send_ready) {
...
int zmq::sspi_client_t::process_handshake_command (msg_t *msg_)
{
    if (state == recv_ready) {
...
int zmq::sspi_client_t::encode (msg_t *msg_)
{
...
int zmq::sspi_client_t::decode (msg_t *msg_)
{
    zmq_assert (state == connected);
...
zmq::mechanism_t::status_t zmq::sspi_client_t::status () const
{
    return state == connected ? mechanism_t::ready : mechanism_t::handshaking;
}

int zmq::sspi_client_t::initialize_context ()
{
...
int zmq::sspi_client_t::process_next_token (msg_t *msg_)
{
