...
//  This test requires a SSPI environment with the following
...
std::string sspi_principal_name ()
{
    std::string buffer;
    buffer.resize(256);
...
std::string 
    principal_name = sspi_principal_name (),
    target_name = "jeh/DESKTOP-8Q1VIQK";
                       ...
                       char *endpoint_)
{
    ...
    assert (client);
    int rc = zmq_setsockopt (client, ZMQ_GSSAPI_SERVICE_PRINCIPAL, target_name.c_str(),
                             target_name.size());
    assert (rc == 0);
    rc = zmq_setsockopt (client, ZMQ_GSSAPI_PRINCIPAL, principal_name.c_str(), principal_name.size());
    assert (rc == 0);
...
int main (int argc,char **argv)
{
    if(argc==2)
        target_name=argv[1];
    std::cout
        <<"principal: "<<principal_name<<std::endl
    ...
    assert (rc == 0);
    rc = zmq_setsockopt (server, ZMQ_GSSAPI_PRINCIPAL, principal_name.c_str(), principal_name.size());
    assert (rc == 0);
    int name_type = ZMQ_GSSAPI_NT_HOSTBASED;
